...
export function ApartmentCard({
  ...
  const images: Record<string, string> = useMemo(() => ({
    Dream: 'https://res.cloudinary.com/dfu9nzn8r/image/upload/f_auto,q_auto,w_1600/v1771578261/Cucinafinestraaperta_neez2f.jpg',
    Heaven: 'https://res.cloudinary.com/dfu9nzn8r/image/upload/f_auto,q_auto,w_1600/v1771578387/701821812_bxr9dq.jpg',
    Oasis: 'https://res.cloudinary.com/dfu9nzn8r/image/upload/f_auto,q_auto,w_1600/v1771578489/701821932_vg3nmu.jpg'
  }), []);
  ...
  return <div ref={cardRef} className={`relative max-w-sm w-full ${className}`} style={{
      ...
      <div role="button" tabIndex={0} onClick={toggle} className="outline-none cursor-pointer select-none">
        ...
        transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)'
          ...
          transform: 'translateZ(0)'
            ...
            <img src="https://res.cloudinary.com/dfu9nzn8r/image/upload/v1771674668/IMG_0326_eywbni.png" alt="stamp" className="absolute top-4 right-4 w-20 h-20 object-contain pointer-events-none select-none" draggable={false} />
          ...
          transform: 'rotateY(180deg) translateZ(0)'
            ...
            {imageUrl && <img src={imageUrl} alt={name} className="w-full h-full object-cover" loading="lazy" draggable={false} />}
...
export function Apartments() {
  const apartments = [{
    name: 'Dream',
    description: 'A serene retreat with a private terrace overlooking the olive groves. Wake up to birdsong and the scent of Mediterranean herbs.',
    features: ['3 Bedrooms · 4+2 Max Occ.', 'Private terrace', 'Full kitchen', 'Air conditioning', 'Two patios (one with pool view)'],
    rotation: '-1deg'
  }, {
    name: 'Heaven',
    description: 'Light-filled and spacious, Heaven opens onto the shared patio and pool. Perfect for families seeking comfort and connection.',
    features: ['2 Bedrooms · 4+1 Max Occ.', 'Patio & pool access', 'Full kitchen', 'Air conditioning', 'Garden views'],
    rotation: '0.5deg'
  }, {
    name: 'Oasis',
    description: 'The most intimate of the three, Oasis is a cozy hideaway for couples or small families, with its own quiet corner of the garden.',
    features: ['1 Bedrooms · 4 Max Occ.', 'Patio & pool access', 'Full kitchen', 'Air conditioning', 'Garden views'],
    rotation: '1.5deg'
  }];
  return <section className="py-24 px-4 bg-paper-texture-dark scroll-mt-24">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-center items-start gap-10 md:gap-8">
          {apartments.map((apt) => <ApartmentCard key={apt.name} name={apt.name} description={apt.description} features={apt.features} rotation={apt.rotation} />)}
        </div>
      </div>
    </section>;
}